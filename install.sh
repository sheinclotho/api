...
elif [ -f "./${REPO_NAME}/web.py" ]; then
    log_info "Changing to ${REPO_NAME} directory"
    cd "./${REPO_NAME}" || exit 1
else
    log_info "Cloning repository..."
    if [ -d "./${REPO_NAME}" ]; then
        log_warn "${REPO_NAME} directory exists but web.py not found. Removing and re-cloning..."
        rm -rf "./${REPO_NAME}"
    fi
    
    if ! git clone https://github.com/sheinclotho/api.git; then
