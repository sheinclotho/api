...
from src.router.vertex.anthropic import router as vertex_anthropic_router
...
# Vertex AI路由 - 处理OpenAI格式请求并转换为Vertex AI API
app.include_router(vertex_openai_router, prefix="", tags=["Vertex AI OpenAI API"])

# Vertex AI路由 - 处理Gemini格式请求并转换为Vertex AI API
app.include_router(vertex_gemini_router, prefix="", tags=["Vertex AI Gemini API"])

# Vertex AI模型列表路由 - 处理Gemini格式的模型列表请求
app.include_router(vertex_model_list_router, prefix="", tags=["Vertex AI Model List"])

# Vertex AI Anthropic Messages 路由 - Anthropic Messages 格式兼容
app.include_router(vertex_anthropic_router, prefix="", tags=["Vertex AI Anthropic Messages"])
...
async def main():
    ...
    log.info(f"   Vertex AI (OpenAI格式): http://127.0.0.1:{port}/vertex/v1")
    log.info(f"   Vertex AI (Claude格式): http://127.0.0.1:{port}/vertex/v1")
    log.info(f"   Vertex AI (Gemini格式): http://127.0.0.1:{port}/vertex")
