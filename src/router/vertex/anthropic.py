"""
Anthropic Router - Handles Anthropic/Claude format API requests via Vertex AI
通过Vertex AI处理Anthropic/Claude格式请求的路由模块
"""
...
@router.post("/vertex/v1/messages")
async def messages(
    ...
    Args:
        ...
        token: Bearer认证令牌
    """
    log.debug(f"[VERTEX-ANTHROPIC] Request for model: {claude_request.model}")
...
@router.post("/vertex/v1/messages/count_tokens")
async def count_tokens(
    ...
    log.info(
        f"[VERTEX-ANTHROPIC] /messages/count_tokens 收到请求: client={client_host}:{client_port}, "
    ...
    try:
        ...
        log.error(f"[VERTEX-ANTHROPIC] token 估算失败: {e}")
