...
async def generate_content(
    ...
    Args:
        ...
        api_key: API 密钥
    """
    log.debug(f"[VERTEX] Non-streaming request for model: {model}")
...
async def stream_generate_content(
    ...
    Args:
        ...
        api_key: API 密钥
    """
    log.debug(f"[VERTEX] Streaming request for model: {model}")
                ...
                if chunk_str.startswith("data: "):
                    ...
                    try:
                        ...
                        if "response" in data and "candidates" not in data:
                            log.debug(f"[VERTEX-ANTI-TRUNCATION] 展开response包装")
    ...
    async def normal_stream_generator():
        ...
        log.debug(f"[VERTEX] 使用非native模式，将展开response包装")
        ...
        async for chunk in stream_gen:
            ...
            if isinstance(chunk, Response):
                ...
                log.error(f"[VERTEX STREAM] 返回错误给客户端: status={chunk.status_code}, error={str(error_json)[:200]}")
            ...
            if isinstance(chunk, (str, bytes)):
                ...
                if chunk_str.startswith("data: "):
                    ...
                    try:
                        ...
                        # 展开 response 包装
                        if "response" in data and "candidates" not in data:
                            log.debug(f"[VERTEX] 展开response包装")
                            unwrapped_data = data["response"]
                            # 重新构建SSE格式
                            yield f"data: {json.dumps(unwrapped_data, ensure_ascii=False)}\n\n".encode('utf-8')
                        else:
